...
#ifdef ENABLE_CHAT
		if (pChatLine->EmpireFlag && pChatLine->iType == CHAT_TYPE_SHOUT)
			pChatLine->EmpireFlag->SetPosition(pChatSet->m_ix, pChatSet->m_iy + iHeight + 2);
#endif
	}
}
...
void CPythonChat::Render(DWORD dwID)
{
	...
	for (TChatLineList::iterator itor = pLineList->begin(); itor != pLineList->end(); ++itor)
	{
		...
		if (pChatLine->EmpireFlag && pChatLine->iType == CHAT_TYPE_SHOUT)
		{
			CGraphicImageInstance*& imInstance = (*itor)->EmpireFlag;
			if (imInstance)
				imInstance->Render();
		}
#endif
...
void CPythonChat::AppendChat(int iType, const char* c_szChat)
{
	...
	{
		CGraphicImageInstance*& prFlag = pChatLine->EmpireFlag;
		...
		{
			char szPath[256];
			sprintf(szPath, "d:/ymir work/ui/tunga/flag/%s.tga", empire);
