...
int get_Item_SubType_Value(unsigned int type_value, string inputString)
{
	...
	static string* arSubType[] =
	{
		...
		0,			//33 
		0,			//34 
		0,			//35//USE_CHANGE_COSTUME_ATTR
		0,			//36//USE_RESET_COSTUME_ATTR
		0,			//37//USE_UNK33
		0,			//38//USE_CHANGE_ATTRIBUTE_PLUS
	};
	...
	if (arSubType[type_value] == 0) { return 0; }
	//
...
bool Set_Proto_Item_Table(TItemTable* itemTable, cCsvTable& csvTable, std::map<int, const char*>& nameMap)
{
	...
	itemTable->bSize = MINMAX(1, dataArray[4], 3);/* @Turkmmo67
