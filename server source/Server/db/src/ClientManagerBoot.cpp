	...
	for (itertype(m_vec_itemTable) it = m_vec_itemTable.begin(); it != m_vec_itemTable.end(); it++)
	{
		if (g_stLocaleNameColumn != "name")
		{
			...
			snprintf(query, sizeof(query),
				...
				t.aApplies[0].bType, t.aApplies[0].lValue, t.aApplies[1].bType, t.aApplies[1].lValue, t.aApplies[2].bType, t.aApplies[2].lValue, t.aApplies[3].bType, t.aApplies[3].lValue,//30
...
bool CClientManager::InitializeItemTableFromDB()
{
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		item_table->bSize = MINMAX(1, item_table->bSize, 3);//@turkmmo67
